...
nameSpaces = [
    nmSp for nmSp in cmds.namespaceInfo(listOnlyNamespaces=True, recurse=True)
    if nmSp not in defaults
]

# Reverse The List, So Name Spaces With More Children Comes On First.
nameSpaces.sort(key=numChildren,
                reverse=True)  # Skip This Line If Need To Delete One By One
...
for name in nameSpaces:
    ...
    cmds.namespace(removeNamespace=name,
                   ...
                   force=True)  # Name Spaces Must Be Empty For Removal
