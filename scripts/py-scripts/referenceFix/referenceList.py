...
def LoadMayaFilesDir(*args):
    fDir = cmds.fileDialog2(dialogStyle=2, caption='Select Maya Files Directory', fileMode=3, okCaption='Accept', returnFilter=True)[0]
    cmds.textFieldGrp(fText, text = fDir, e=1)


def ReferenceFilesDir(*args):
    rDir = cmds.fileDialog2(dialogStyle=2, caption='Select Reference Files Directory', fileMode=3, okCaption='Accept', returnFilter=True)[0]
...
def RunButtonPush(*args):
    ...
    # For Text Generation
        if currentValue == 1:
         ...
         cmds.inViewMessage(assistMessage='<h1>Please Provide Valid Path</h1>', position='midCenter', font="Cascadia Mono SemiBold", fade=True, alpha=0.0)   
        



# Creates Actual Window
window = cmds.window(winID, title='Reference Error Checker | v' + str(version), width=375, height=180, tlb = True, sizeable = False)
...
cmds.radioButtonGrp('Radio_Type', labelArray3=['Generate TXT', 'Find/Replace', 'Console Print'], numberOfRadioButtons=3, enable=True, select=1)
cmds.setParent('..')
cmds.separator(h=5)
cmds.rowLayout(numberOfColumns=4)
